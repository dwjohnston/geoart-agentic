...
export function generateComputeRegistryContent(
...
export function generateControlRegistryContent(
	nodeBasenames: string[],
): string {
	const imports = nodeBasenames
		.map((b) => `import ${b} from './nodes/${b}';`)
...
function scanNodeFiles(dir: string, ext: string): string[] {
	return fs
		...
		.filter(
			(f) =>
				f.endsWith(ext) && !f.includes(".test.") && !f.includes(".snapshot."),
		)
		.sort()
...
if (isMain) {
	...
	const computeBasenames = scanNodeFiles(
		path.join(nodesDir, "compute/nodes"),
		".ts",
	);
	const renderBasenames = scanNodeFiles(
		path.join(nodesDir, "render/nodes"),
		".ts",
	);
	const controlBasenames = scanNodeFiles(
		path.join(nodesDir, "control/nodes"),
		".tsx",
	);

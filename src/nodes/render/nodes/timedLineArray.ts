...
function computeLinks(
	...
	if (mode === "distribute") {
		return arrayA.map(
			(a, i): Link => [
				a,
				arrayB[Math.floor((i * arrayB.length) / arrayA.length)],
			],
		);
	}
...
function getInsideOutLinksAtStep(links: Link[], step: number): Link[] {
	...
	if (rightIdx !== leftIdx && rightIdx >= 0 && rightIdx < n)
		result.push(links[rightIdx]);
	return result;
}
...
function drawLink(
	...
	gradient.addColorStop(
		0,
		`rgba(${a.r * 255}, ${a.g * 255}, ${a.b * 255}, ${a.a})`,
	);
	gradient.addColorStop(
		1,
		`rgba(${b.r * 255}, ${b.g * 255}, ${b.b * 255}, ${b.a})`,
	);
...
function selectLinks(
	...
	const state = (ctx.getState?.() as IntervalState | undefined) ?? {
		counter: 0,
	};

...
describe('orbitNodeImplementation', () => {
  it('at t=0, point is at (radius, 0)', () => {
    const { point } = orbitNodeImplementation.evaluate(base);
    expect(point.x).toBeCloseTo(0.5);
    expect(point.y).toBeCloseTo(0);
  });

  it('at t=150 (quarter turn), point is at (0, radius)', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, time: 150 });
    expect(point.x).toBeCloseTo(0);
    expect(point.y).toBeCloseTo(0.5);
  });

  it('at t=300 (half turn), point is at (-radius, 0)', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, time: 300 });
    expect(point.x).toBeCloseTo(-0.5);
    expect(point.y).toBeCloseTo(0);
  });

  it('at t=600 (full turn), point returns to start', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, time: 600 });
    expect(point.x).toBeCloseTo(0.5);
    expect(point.y).toBeCloseTo(0);
  });

  it('speed=2 doubles angular velocity', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, speed: 2, time: 150 });
    expect(point.x).toBeCloseTo(-0.5);
    expect(point.y).toBeCloseTo(0);
  });

  it('radius=0 always returns origin', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, radius: 0, time: 100 });
    expect(point.x).toBeCloseTo(0);
    expect(point.y).toBeCloseTo(0);
  });

  it('center offset shifts the point', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, center: { x: 0.3, y: 0.2 } });
    expect(point.x).toBeCloseTo(0.8);
    expect(point.y).toBeCloseTo(0.2);
  });

  it('phase shifts starting angle', () => {
    const { point } = orbitNodeImplementation.evaluate({ ...base, phase: 0.25 });
    expect(point.x).toBeCloseTo(0);
    expect(point.y).toBeCloseTo(0.5);
  });

  it('numPoints=4 returns four evenly-spaced colour points', () => {
    const { points } = orbitNodeImplementation.evaluate({ ...base, numPoints: 4 });
  ...
  it('colour points have white colour by default', () => {
    const { points } = orbitNodeImplementation.evaluate(base);
    expect(points[0]).toMatchObject({ r: 1, g: 1, b: 1, a: 1 });
  });

  it('numPoints=2 returns two points 180 degrees apart', () => {
    const { points } = orbitNodeImplementation.evaluate({ ...base, numPoints: 2 });
  ...
  it('tangent at t=0 points upward (perpendicular to rightmost radius)', () => {
    const { points } = orbitNodeImplementation.evaluate({ ...base, phase: 0, numPoints: 1 });
    ...
    const result2 = orbitNodeImplementation.evaluate({ ...base, phase: 0, numPoints: 1, time: 1, speed: 600 / 4 });
  ...
  describe('eccentricity', () => {
    it('eccentricity=0 produces a circle (default behaviour)', () => {
      const { point } = orbitNodeImplementation.evaluate({ ...base, time: 150, eccentricity: 0 });
      expect(point.x).toBeCloseTo(0);
      expect(point.y).toBeCloseTo(0.5);
    });

    it('eccentricity=1 collapses the orbit to a flat line (y=0 for all t)', () => {
      const atZero = orbitNodeImplementation.evaluate({ ...base, time: 0, eccentricity: 1 });
      const atQuarter = orbitNodeImplementation.evaluate({ ...base, time: 150, eccentricity: 1 });
      const atHalf = orbitNodeImplementation.evaluate({ ...base, time: 300, eccentricity: 1 });
    ...
    it('eccentricity=1 at t=0 puts point at (radius, 0)', () => {
      const { point } = orbitNodeImplementation.evaluate({ ...base, eccentricity: 1 });
      expect(point.x).toBeCloseTo(0.5);
      expect(point.y).toBeCloseTo(0);
    });

    it('eccentricity=0.5 squashes the y-axis to half radius at the quarter turn', () => {
      const { point } = orbitNodeImplementation.evaluate({ ...base, time: 150, eccentricity: 0.5 });
      expect(point.x).toBeCloseTo(0);
  ...
  describe('centerPoints', () => {
    it('3 centre points with numPoints=2 produces 6 output points', () => {
      ...
      const { points } = orbitNodeImplementation.evaluate({ ...base, centerPoints: centres, numPoints: 2 });
    ...
    it('output points inherit colour from their respective centre point', () => {
      ...
      const { points } = orbitNodeImplementation.evaluate({ ...base, centerPoints: centres, numPoints: 1 });
      expect(points).toHaveLength(2);
    ...
    it('centerPoints takes precedence over center when both are provided', () => {
      ...
      const { points } = orbitNodeImplementation.evaluate({ ...base, centerPoints: centres, center: { x: 0.9, y: 0.9 }, numPoints: 1 });
    ...
    it('empty centerPoints falls back to the center input', () => {
      const { points } = orbitNodeImplementation.evaluate({ ...base, centerPoints: [], center: { x: 0.3, y: 0.2 }, numPoints: 1 });
      expect(points).toHaveLength(1);
  ...
  describe('tilt', () => {
    it('tilt=0 produces no rotation (default behaviour)', () => {
      const { point } = orbitNodeImplementation.evaluate({ ...base, tilt: 0 });
      expect(point.x).toBeCloseTo(0.5);
      expect(point.y).toBeCloseTo(0);
    });

    it('tilt=0.25 rotates the orbit 90 degrees — t=0 point moves to (0, radius)', () => {
      const { point } = orbitNodeImplementation.evaluate({ ...base, tilt: 0.25 });
      expect(point.x).toBeCloseTo(0);
      expect(point.y).toBeCloseTo(0.5);
    });

    it('tilt=0.5 rotates 180 degrees — t=0 point moves to (-radius, 0)', () => {
      const { point } = orbitNodeImplementation.evaluate({ ...base, tilt: 0.5 });
      expect(point.x).toBeCloseTo(-0.5);
      expect(point.y).toBeCloseTo(0);
    });

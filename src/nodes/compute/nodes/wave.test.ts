...
} satisfies NodeInputsResolved<"wave">;
...
describe("waveNodeDef", () => {
	describe("sine", () => {
		...
		test("amplitude scales output", () => {
			expect(
				waveNodeDef.evaluate({ ...base, amplitude: 0.5, time: 15 }).value,
			).toBeCloseTo(0.5);
		});
		test("phase 0.25 shifts peak to t=0", () => {
			expect(waveNodeDef.evaluate({ ...base, phase: 0.25 }).value).toBeCloseTo(
				1,
			);
		});
		test("phase 0.5 inverts", () => {
			expect(
				waveNodeDef.evaluate({ ...base, phase: 0.5, time: 15 }).value,
			).toBeCloseTo(-1);
		});
	});
	...
	describe("square", () => {
		...
		test("amplitude scales output", () => {
			expect(
				waveNodeDef.evaluate({ ...sq, amplitude: 0.5, time: 6 }).value,
			).toBe(0.5);
		});
	});
	...
	describe("saw", () => {
		...
		test("approaches +1 near end of cycle", () => {
			expect(waveNodeDef.evaluate({ ...saw, time: 59.94 }).value).toBeCloseTo(
				1,
				1,
			);
		});
	...
	describe("reverse-saw", () => {
		...
		test("approaches -1 near end of cycle", () => {
			expect(waveNodeDef.evaluate({ ...rsaw, time: 59.94 }).value).toBeCloseTo(
				-1,
				1,
			);
		});
	...
	describe("triangle", () => {
		...
		test("amplitude scales output", () => {
			expect(
				waveNodeDef.evaluate({ ...tri, amplitude: 2, time: 15 }).value,
			).toBeCloseTo(2);
		});
	});
	...
	describe("sampler", () => {
		test("sine: base case - does a regular cycle in 0-1", () => {
			...
			const result = waveNodeDef.evaluate({
				...base,
				frequency: 1,
				amplitude: 1,
				time: 0,

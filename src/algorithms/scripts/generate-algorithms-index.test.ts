...
import { filePathToId, generateAlgorithmsIndexContent } from './generate-algorithms-index';

describe('filePathToId', () => {
  test('strips ReferenceGraph suffix and converts to kebab-case', () => {
    expect(filePathToId('./reference/canonical/colorShiftOrbitReferenceGraph.ts')).toBe('color-shift-orbit');
    expect(filePathToId('./reference/general/earthVenus.ts')).toBe('earth-venus');
    expect(filePathToId('./reference/minimal/minimalThreeNodeReferenceGraph.ts')).toBe('minimal-three-node');
  });
});
...
describe('generateAlgorithmsIndexContent', () => {
  test('generates imports, rawEntries, GRAPHS, and getGraph', () => {
    ...
    expect(content).toContain("import earthVenus from './reference/general/earthVenus';");
    expect(content).toContain(

...
function fixInternalRefs(
	...
	if (
		...
		result.$ref.startsWith("#/definitions/")
	) {
		result.$ref = `value-kinds.schema.json${result.$ref}`;
	}

	for (const key of Object.keys(result)) {
		result[key] = fixInternalRefs(
			result[key] as Record<string, unknown> | unknown[],
			currentDepth + 1,
		);
	}
...
export function extractDerviedSchema(schema: any) {
	...
	for (const defName of Object.keys(schema.definitions)) {
		...
		if (isArrayType) {
			...
			orRefDef.properties.v.items = {
				oneOf: [fixInternalRefs(baseItemRef), refParamRef],
			};

			definitions[`${defName}OrRef`] = {
				title: `${def.title} Or Ref`,
				oneOf: [fixInternalRefs(orRefDef), refParamRef],
			};
		} else {
			definitions[`${defName}OrRef`] = {
				title: `${def.title} Or Ref`,
	...
	return {
		...
		description:
			"Param-level definitions: each value kind as either a static value or a ref to another node's output",
		definitions,
	};
}
...
if (isMain) {
	try {
		const schema = await import("../schema/value-kinds.schema.json", {
			with: { type: "json" },
		});
		...
		const outFile = resolve(
			import.meta.dirname,
			"../schema/refable-value-kinds.schema.json",
		);

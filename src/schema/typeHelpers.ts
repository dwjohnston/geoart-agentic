...
export type ValueTypeNameToValueType<T extends ValueTypeNames> = Extract<
	ValueTypes,
	{ kind: T }
>;
...
export type ResolvedValue<T extends ValueTypeNamesSuffixed> =
	ValueTypeByName<T> extends { v: infer V } ? V : never;

export type InputDefsAsObject<T extends keyof typeof nodeInputs> =
	(typeof nodeInputs)[T];
...
export type NodeOutputsResolved<K extends keyof typeof nodeOutputMeta> = {
	[Item in (typeof nodeOutputMeta)[K][number] as Item["name"]]: ValueTypeByName<
		Item["valueType"]
	> extends { v: infer V }
		? V
		: never;
...
export type NodeInputsResolved<K extends keyof typeof nodeInputs> = {
	[Port in keyof (typeof nodeInputs)[K]]: (typeof nodeInputs)[K][Port] extends {
		valueType: infer VT extends ValueTypeNamesSuffixed;
	}
		? ResolvedValue<VT>
		: never;
};
...
type IsArrayValueType<T extends ValueTypeNames> = T extends `${string}Array`
	? true
	: false;

// Extract the item type from an array value kind (e.g., "colorPointArray" -> "colorPoint")
type ArrayItemType<T extends ValueTypeNames> = T extends `${infer Item}Array`
	? Item
	: never;

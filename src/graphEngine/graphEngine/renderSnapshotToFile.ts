...
const SNAPSHOT_PATH = join(__dir, '__snapshots__/allReferenceGraphs.snapshot.test.ts.snap');
...
function parseSnapshot(content: string, testName: string): Call[] {
  ...
  if (keyIndex === -1) throw new Error(`Snapshot not found for test: "${testName}"`);
...
function detectCanvasSize(calls: Call[]): { width: number; height: number } {
  const clearRect = calls.find(
    (c): c is Extract<Call, { kind: 'method' }> => c.kind === 'method' && c.name === 'clearRect',
  );
  if (clearRect && clearRect.args.length >= 4) {
    return { width: clearRect.args[2] as number, height: clearRect.args[3] as number };
  }
...
export function renderSnapshotToFile(testName: string): string {
  ...
  const buffer = replayCallsOnCanvas(calls, width, height).toBuffer('image/png');

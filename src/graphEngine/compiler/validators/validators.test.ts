...
describe('DUPLICATE_NODE_ID', () => {
  test('same id in two different layers is an error', () => {
    const graph: GeoArtGraph = {
      version: '1.0',
      control: {
        nodes: [{ id: 'shared', type: 'slider', params: { value: { v: 0.5 } } }],
      },
      compute: {
    ...
    const graph: GeoArtGraph = {
      ...
      compute: {
        nodes: [
          ...
          { id: 'nodeA', type: 'orbit', params: { time: { ref: 'nodeB.point' } } },
          { id: 'nodeB', type: 'orbit', params: { time: { ref: 'nodeA.point' } } },
        ],
      },
      render: { nodes: [] },
    };
...
describe('TYPE_MISMATCH', () => {
  ...
  test('correct type wiring produces no type mismatch', () => {
    ...
    const result = validateGraphSemantics(graph);
    expect(result.errors.filter(e => e.code === 'TYPE_MISMATCH')).toHaveLength(0);
  });
});
...
describe('INVALID_ENUM_VALUE', () => {
  test('valid graph with no enum ports produces no enum errors', () => {
    ...
    const result = validateGraphSemantics(earthVenus);
    expect(result.errors.filter(e => e.code === 'INVALID_ENUM_VALUE')).toHaveLength(0);
  });
});
...
describe('ORPHANED_NODE', () => {
  test('a compute node not connected to any render node is a warning', () => {
    ...
    const orphanErrors = result.errors.filter(e => e.code === 'ORPHANED_NODE');
  ...
  test('orphaned node warning does not make the graph invalid', () => {
    const graph: GeoArtGraph = {
      version: '1.0',
      control: {
        nodes: [{ id: 'unusedSlider', type: 'slider', params: { value: { v: 0.5 } } }],
      },
      compute: {
        nodes: [
          { id: 'time', type: 'time', params: {} },
    ...
    expect(result.errors.some(e => e.code === 'ORPHANED_NODE' && e.nodeId === 'unusedSlider')).toBe(true);
  });

...
function paramToValue(v: unknown): Value {
	...
	throw new Error(
		`Cannot convert param value to internal Value: ${JSON.stringify(v)}`,
	);
}
...
export function compile(
	...
	for (const node of graph.control.nodes) {
		...
		if (!def) {
			throw new Error(
				`Unknown control node type: "${node.type}" (id: "${node.id}")`,
			);
		}
	...
	for (const node of graph.compute.nodes) {
		...
		if (!def) {
			throw new Error(
				`Unknown compute node type: "${node.type}" (id: "${node.id}")`,
			);
		}
		const rawParams = (node.params ?? {}) as Record<string, unknown>;
		rawParamsByNodeId.set(node.id, rawParams);
	...
	for (const node of graph.render.nodes) {
		...
		if (!def) {
			throw new Error(
				`Unknown render node type: "${node.type}" (id: "${node.id}")`,
			);
		}
		const rawParams = (node.params ?? {}) as Record<string, unknown>;
		rawParamsByNodeId.set(node.id, rawParams);
	...
	for (const [toNodeId, compiledNode] of nodes.entries()) {
		...
		const inputs = ((def as LegacyComputeNodeDef).inputs ??
			(def as LegacyRenderNodeDef).inputs) as
			| LegacyComputeNodePortDef[]
			| undefined;
		if (!inputs) continue;
		...
		for (let toPort = 0; toPort < inputs.length; toPort++) {
			...
			const fromDef = fromCompiledNode.def as
				| LegacyComputeNodeDef
				| LegacyControlNodeDef
				| LegacyRenderNodeDef;

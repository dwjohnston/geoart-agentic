...
function paramToValue(v: unknown): Value {
  ...
  if (Array.isArray(inner)) {
    ...
    if (records.every(item => 'x' in item && 'y' in item && 'r' in item && 'g' in item && 'b' in item && 'a' in item)) {
      return {
        kind: 'colorPointArray',
        v: records as Array<{ x: number; y: number; r: number; g: number; b: number; a: number }>,
      };
    }

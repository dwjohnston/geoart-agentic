...
export function createImportNoRestrictedPathsZones<T extends ZoneNameToZonePathMap>(folderConfigMapping: T, allowMappings: AllowedMappingConfig<T>): Array<{
    from: string;
    target: string;
    message: string;
}> {
    ...
    return entries.flatMap((v) => {
        ...
        const disallowedZones = [...zoneNamesAsSet.difference(new Set(allowedZones))]

        return disallowedZones.map((w) => {
